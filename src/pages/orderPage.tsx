...
const OrderPage: FC = () => {
  ...
  const classText =
    windowSize.width > 850 ? 'text_type_main-default' : 'text_type_main-small';
  ...
  const [qtyOfEachIngredient, setQtyOfEachIngredient] = useState<
  ...
  useEffect(() => {
    if (
      ...
      for (let id in qtyOfEachIngredient) {
        ...
        const foundedObj = findIngredientObj(
          id,
          ingredientsFromServer
        ) as TIngredientExtraId;
    ...
    return (
      <>
        <main className={styles.main}>
          <p
            className={`text text_type_digits-default text_centered ${styles.number}`}
          >
            #{order.number}
          </p>
          <h2
            className={`text ${classTextHeading} ${marginForHeading} ${styles.name} ${scrollStyle}`}
          >
            {order.name}
          </h2>
          <p
            className={`text ${classText} ${order.status === StatusKind.DONE ? styles.success : ''}`}
          >
            {statusRus}
          </p>

          <h3 className={`text ${classTextHeading} ${marginForCompound}`}>
            Состав:
          </h3>
          ...
          <div className={styles.footer}>
            <FormattedDate
              ...
              className={`text ${classText} text_color_inactive`}
            />
            <Price
              value={ingredientsArr.reduce((acc, el) => acc + el!.price, 0)}
