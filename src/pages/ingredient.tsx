...
const IngredientPage = () => {
  ...
  useEffect(() => {
    if (ingredientsData.length && id)
      setIngredient(findIngredientObj(id, ingredientsData));
  }, [id, ingredientsData]);
  ...
  return (
    <main className={`${styles.container} pl-2 pr-2`}>
      <h1 className={`text ${textClassForHeading} ${marginTopForHeading}`}>
        Детали ингредиента
      </h1>

      <img src={image_large || image} alt={name} className={styles.image} />

      <h2 className={`text text_type_main-medium text_centered`}>{name}</h2>

      <ul className={styles.list}>
        <li className={styles.item}>
          <h3
            className={`text ${sizeForNames} text_color_inactive text_centered`}
          >
            Калории, ккал
          </h3>
          <p
            className={`text ${sizeForDigits} text_color_inactive text_centered`}
          >
            {calories}
          </p>
        </li>
