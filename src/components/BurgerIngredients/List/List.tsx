...
const List = forwardRef((props, ref: React.ForwardedRef<TSuperRef>) => {
  ...
  return (
    <>
      ...
      <div
        className={`${windowSize.width < 501 ? 'custom-scroll_nullish' : 'custom-scroll'} ${windowSize.width > 500 ? 'mt-10' : 'mt-5'} ${styles.section}`}
        ref={listRef}
      >
        {isLoading ? (
          ...
          <>
            <h3
              className={`text ${classTypographyForHeader} ${styles.heading}`}
              id="buns"
              ref={bunsRef}
            >
              Булки
