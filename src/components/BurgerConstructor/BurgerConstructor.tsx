...
function BurgerConstructor() {
  ...
  return (
    <>
      ...
      <section
        className={`${styles.sectionCommon} pt-25 pb-5`}
        ref={sectionElem}
      >
        <section
          ref={dropRef}
          className={
            isHover
              ? `${styles.sectionConstructor} ${styles.dropTarget}`
              : styles.sectionConstructor
          }
        >
          {!selectedBun && selectedProducts.length === 0 && (
            <div className={styles.stub}>
              ...
              <p
                className={`text text_type_main-default text_color_inactive ${styles.stubText}`}
              >
          ...
          <ul
            className={`${styles.listOfFillings} custom-scroll`}
            ref={fillingsElem}
          >
            {selectedProducts.length > 0 &&
              selectedProducts.map((el, index) => (
                <Item key={el.extraId} ingredient={el} index={index} />
              ))}
          </ul>
