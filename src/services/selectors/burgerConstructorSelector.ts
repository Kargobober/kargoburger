...
export const getIsBunNullish = (state: RootState) =>
  !Boolean(state.burgerConstructor.selectedBun);
export const getSelectedProducts = (state: RootState) =>
  state.burgerConstructor.selectedProducts;
export const getTotalPrice = (state: RootState) => {
  ...
  if (state.burgerConstructor.selectedProducts.length > 0) {
    fillingsPrice = state.burgerConstructor.selectedProducts.reduce(
      (acc, el) => acc + el.price,
      0
    );
  }
