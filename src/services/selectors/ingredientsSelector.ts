...
export const getCountedFiltredIngredients = (state: RootState) => {
  ...
  const bunsFromServer = state.ingredients.ingredientsData.filter(
    (el) => el.type === 'bun'
  );
  ...
  const bunsCounted: TIngredientCounted[] = bunsFromServer.map((item) => {
    ...
    return {
      ...item,
      qty:
        state.burgerConstructor.selectedBun == null
          ? 0
          : state.burgerConstructor.selectedBun._id === item._id
            ? 2
            : 0,
    };
  ...
  const saucesFromServer = state.ingredients.ingredientsData.filter(
    (el) => el.type === 'sauce'
  );
  const saucesCounted: TIngredientCounted[] = saucesFromServer.map((item) => {
    return {
      ...item,
      qty: state.burgerConstructor.selectedProducts.reduce(
        (acc, selectedProduct) => {
  ...
  const mainFillingsFromServer = state.ingredients.ingredientsData.filter(
    (el) => el.type === 'main'
  );
  const mainFillingsCounted: TIngredientCounted[] = mainFillingsFromServer.map(
    (item) => {
      return {
        ...item,
        qty: state.burgerConstructor.selectedProducts.reduce(
